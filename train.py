...
def main(args):
    ...
    for epoch in range(config.num_epochs):
        ...
        with torch.no_grad():
            ...
            test_loss = mse
            mse += config.prior_weight * kl_prior.clamp(min=1.0)
            mse += config.posterior_weight * kl_posterior.clamp(min=1.0)
        ...
        for i in tqdm(range(train_size // config.batch_size)):
            ...
            loss = mse
            loss += config.prior_weight * kl_prior.clamp(min=1.0)
            loss += config.posterior_weight * kl_posterior.clamp(min=1.0)
